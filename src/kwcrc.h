 ...
 * 模块概述
 * ========
 * 本模块是 LoRaWAN 基站的关键字 CRC 定义文件，为 JSON 处理系统提供高效的字符串查找功能。
 * 它包含了预计算的 CRC32 值，用于快速识别和处理 JSON 字段名及消息类型。
 * 该模块通过哈希值比较而非字符串比较来提高处理效率，是 JSON 解析系统的重要组成部分。
 * 
 * 文件功能
 * ========
 * 本头文件定义了系统中使用的所有关键字的 CRC32 哈希值：
 * - CRC 计算和完成的宏定义
 * - JSON 字段名的 CRC32 常量
 * - 消息类型的 CRC32 常量
 * - 配置参数名的 CRC32 常量
 * - 区域和设备类型标识符的 CRC32 常量
 * 
 * 主要组件
 * ========
 * 1. CRC 计算宏
 ...
 * 
 * 3. 命名约定
 *    - 所有 CRC32 常量以 J_ 前缀开头
 *    - 常量名与关键字相对应
 *    - 使用全大写指示特殊标识符（如 EUI、DR）
 * 
 * 注意事项
 * ========
 * 1. 自动生成性质
 *    - 不应手动修改此文件
 *    - 更新关键字应修改 kwlist.txt 并重新生成
 *    - 保持生成工具和此文件的同步
 * 
 * 2. 哈希冲突
 *    - CRC32 可能存在理论上的冲突风险
